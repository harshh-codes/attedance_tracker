...
import { ShieldCheck, Mail, Phone, Key, Calendar, Edit3, Lock, Save, Loader2, CheckCircle2, AlertCircle } from 'lucide-react';
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 pb-6 border-b border-slate-800">
          {user?.profilePhoto ? (
            <img
              ...
              alt={user.firstName}
              className="w-24 h-24 rounded-2xl object-cover border-2 border-amber-500/40 shadow-xl shrink-0"
            />
          ) : (
            <div className="w-24 h-24 rounded-2xl bg-amber-500/20 border-2 border-amber-500/40 flex items-center justify-center text-amber-400 font-extrabold text-3xl shadow-xl shrink-0">
          ...
          <div className="text-center sm:text-left flex-1">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
              ...
              <span className="px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-bold uppercase tracking-wider flex items-center gap-1">
                <ShieldCheck className="w-3.5 h-3.5" /> ADMIN ROLE
              </span>
            </div>
            <p className="text-sm font-semibold text-slate-300 mt-1">{user?.designation}</p>
            <p className="text-xs text-amber-400/80 font-medium">{user?.department} Department</p>
          </div>

          <div className="flex sm:flex-col gap-2 w-full sm:w-auto">
            <button
              onClick={() => setIsEditing(!isEditing)}
              className="flex-1 sm:flex-initial px-4 py-2 rounded-xl bg-slate-900 border border-slate-800 hover:border-amber-500/50 text-xs font-semibold text-amber-400 transition-all inline-flex items-center justify-center gap-1.5"
            >
            ...
            <button
              ...
              className="flex-1 sm:flex-initial px-4 py-2 rounded-xl bg-slate-900 border border-slate-800 hover:border-amber-500/50 text-xs font-semibold text-slate-300 transition-all inline-flex items-center justify-center gap-1.5"
            >
          ...
          <form onSubmit={handleSave} className="mt-6 space-y-4 text-xs">
            ...
            <div className="flex items-center justify-end gap-2 pt-4 border-t border-slate-800">
              <button
                type="submit"
                disabled={saving}
                className="px-5 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold flex items-center gap-2"
              >
